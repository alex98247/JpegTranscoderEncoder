...
namespace JpegTranscoderEncoder
{
    ...
    public class JpegDecoder
    {
        ...
        public void njDecodeSOF()
        {
            ...
            for (i = 0; i < nj.ncomp; ++i)
            {
                ...
                if (((c.width < 3) && (c.ssx != ssxmax)) || ((c.height < 3) && (c.ssy != ssymax)))
                    njThrow(nj_result_t.NJ_UNSUPPORTED);
                if ((c.pixels = new byte[c.stride * (nj.mbheight * nj.mbsizey * c.ssy / ssymax)]) == null)
        ...
        public void njDecodeScan()
        {
            ...
            if (nj.posb[nj.pos] != 0 || (nj.posb[nj.pos + 1] != 63) || nj.posb[nj.pos + 2] != 0)
                njThrow(nj_result_t.NJ_UNSUPPORTED);
            njSkip(nj.length);
            for (mbx = mby = 0;;)
            {
        ...
        public void njUpsampleH(nj_component_t c)
        {
            ...
            for (y = c.height; y != 0; --y)
            {
                ...
                for (x = 0; x < xmax; ++x)
                {
                    outv[lout + (x << 1) + 3] = CF(CF4A * c.pixels[lin + x] + CF4B * c.pixels[lin + x + 1] +
                                                   CF4C * c.pixels[lin + x + 2] + CF4D * c.pixels[lin + x + 3]);
                    outv[lout + (x << 1) + 4] = CF(CF4D * c.pixels[lin + x] + CF4C * c.pixels[lin + x + 1] +
                                                   CF4B * c.pixels[lin + x + 2] + CF4A * c.pixels[lin + x + 3]);
                }
        ...
        public void njUpsampleV(nj_component_t c)
        {
            ...
            for (x = 0; x < w; ++x)
            {
                ...
                {
                    outv[cout] = CF(CF4A * c.pixels[cin + -s1] + CF4B * c.pixels[cin] + CF4C * c.pixels[cin + s1] +
                                    CF4D * c.pixels[cin + s2]);
                    cout += w;
                    outv[cout] = CF(CF4D * c.pixels[cin + -s1] + CF4C * c.pixels[cin] + CF4B * c.pixels[cin + s1] +
                                    CF4A * c.pixels[cin + s2]);
                    cout += w;
                    cin += s1;
                }
